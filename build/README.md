...
업무기준 RuleData xlsx(예: 설계KEY C10B1040)에서 비교값이 2개 이상인 `IN` 조건을 비교값 1개당 1행의 `=` 조건으로 쪼갠다.
값이 1개뿐인 IN은 기본적으로 그대로 두고(`--split-single`을 주면 `=`로 변환), 연산자가 `=`인데 비교값에 쉼표로 여러 값이 든 셀도
IN처럼 쪼갠다(`--keep-eq-list`로 끔). 한 행에 쪼갤 조건이 여러 개면 모든 조합(카티션 곱)을 생성한다.
헤더·각주·서식·행 높이는 원본대로 유지하며 `no.`는 1부터 다시 매기고, `--map-xlsx`로 원본 no.↔신규 no. 대응표를 함께 만들 수 있다.
